...
@Component(service = Servlet.class, property = { "sling.servlet.extensions=json",
		"sling.servlet.paths=/bin/hclecomm/cart", "sling.servlet.method=" + HttpConstants.METHOD_GET })
			...
			if (action == null || action.isEmpty() || action == "") {
				...
				if ("view".equalsIgnoreCase(action)) {
					...
					if (payload == null || payload.isEmpty() || payload == "") {
						...
						/*
						 * Sample Payload: payload_add { "cartItem": {
						 * "quote_id": "VbfYhqDHXyDcx1MlTUg9jJzEC3bso2ug",
						 * "sku": "24-MB01", "qty": 14} }
						 * 
						 * payload_update { "cartItem": { "quote_id":
						 * "rMFSYyqeTXGOqj4s7A9QRs9AVaLAMC9T", "sku": "24-MB01",
						 * "qty": 10, "item_id":5} }
						 */
						...
						if (httpResponse.getStatusLine().getStatusCode() == 200) {
							...
							response.getWriter().append(
									"Added items to the cart " + cartId + " with item ID as : " + responseStream);
						} else {
					...
					if (itemId == null || itemId.isEmpty() || itemId == "") {
						response.getWriter()
								.append("No product having item ID " + itemId + " to delete from cart " + cartId);
					} else {
						...
						if (httpResponse.getStatusLine().getStatusCode() == 200) {
							String responseStream = EntityUtils.toString(httpResponse.getEntity());
							response.getWriter().append("Item " + itemId + " has been deleted from cart " + cartId
									+ "with response as " + responseStream);
						} else {
